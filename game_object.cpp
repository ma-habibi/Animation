...
GameObject::GameObject(
    ...
    const char *texture_sheet, 
    int x, int y) {
  obj_texture = TextureManager::load_texture(
      renderer, texture_sheet);
  if (!obj_texture) // Check for loaded texture
    throw std::runtime_error(SDL_GetError());

  // Put default width and height
  SDL_QueryTexture(
      obj_texture, nullptr, nullptr, &w, &h);
...
void GameObject::render(SDL_Renderer *renderer) {
  SDL_RenderCopy(
      renderer, obj_texture, &src_rect, &dst_rect);
}
